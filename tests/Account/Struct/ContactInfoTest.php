...
class ContactInfoTest extends ZimbraTestCase
{
    public function testContactInfo()
    {
        ...
        $xml = <<<EOT
<?xml version="1.0"?>
<result sf="$sortField" exp="true" id="$id" l="$folder" f="$flags" t="$tags" tn="$tagNames" md="$changeDate" ms="$modifiedSequenceId" d="$date" rev="$revisionId" fileAsStr="$fileAs" email="$email" email2="$email2" email3="$email3" type="$type" dlist="$dlist" ref="$reference" tooManyMembers="false" isOwner="true" isMember="false" xmlns:urn="urn:zimbraAccount">
...
EOT;
        ...
        $this->assertEquals($contact, $this->serializer->deserialize($xml, MockContactInfo::class, 'xml'));
    }
}

#[XmlNamespace(uri: 'urn:zimbraAccount', prefix: 'urn')]
class MockContactInfo extends ContactInfo
{
}
