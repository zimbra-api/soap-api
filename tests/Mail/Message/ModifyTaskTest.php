...
use Zimbra\Mail\Message\ModifyTaskEnvelope;
use Zimbra\Mail\Message\ModifyTaskBody;
use Zimbra\Mail\Message\ModifyTaskRequest;
use Zimbra\Mail\Message\ModifyTaskResponse;
...
/**
 * Testcase class for ModifyTask.
 */
class ModifyTaskTest extends ZimbraTestCase
{
    public function testModifyTask()
    {
        ...
        $request = new ModifyTaskRequest(
            $id, $componentNum, $modifiedSequence, $revision, $msg, TRUE, $maxSize, TRUE, TRUE, TRUE
        );
        ...
        $response = new ModifyTaskResponse(
            $calItemId, $deprecatedApptId, $calInvId, $modifiedSequence, $revision, new Id($id), new CalEcho($invite)
        );

        $body = new ModifyTaskBody($request, $response);
        $this->assertSame($request, $body->getRequest());
        $this->assertSame($response, $body->getResponse());
        $body = new ModifyTaskBody();
        $body->setRequest($request)
            ->setResponse($response);
        $this->assertSame($request, $body->getRequest());
        $this->assertSame($response, $body->getResponse());

        $envelope = new ModifyTaskEnvelope($body);
        $this->assertSame($body, $envelope->getBody());
        $envelope = new ModifyTaskEnvelope();
...
<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope" xmlns:urn="urn:zimbraMail">
    <soap:Body>
        <urn:ModifyTaskRequest id="$id" comp="$componentNum" ms="$modifiedSequence" rev="$revision" echo="true" max="$maxSize" want="true" neuter="true" forcesend="true">
...
EOT;
        ...
        $this->assertEquals($envelope, $this->serializer->deserialize($xml, ModifyTaskEnvelope::class, 'xml'));
    }
}
