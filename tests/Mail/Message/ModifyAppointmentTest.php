...
use Zimbra\Mail\Message\ModifyAppointmentEnvelope;
use Zimbra\Mail\Message\ModifyAppointmentBody;
use Zimbra\Mail\Message\ModifyAppointmentRequest;
use Zimbra\Mail\Message\ModifyAppointmentResponse;
...
/**
 * Testcase class for ModifyAppointment.
 */
class ModifyAppointmentTest extends ZimbraTestCase
{
    public function testModifyAppointment()
    {
        ...
        $request = new ModifyAppointmentRequest(
            $id, $componentNum, $modifiedSequence, $revision
        );
        ...
        $response = new ModifyAppointmentResponse(
            $calItemId, $deprecatedApptId, $calInvId, $modifiedSequence, $revision, new Id($id), new CalEcho($invite)
        );

        $body = new ModifyAppointmentBody($request, $response);
        $this->assertSame($request, $body->getRequest());
        $this->assertSame($response, $body->getResponse());
        $body = new ModifyAppointmentBody();
        $body->setRequest($request)
            ->setResponse($response);
        $this->assertSame($request, $body->getRequest());
        $this->assertSame($response, $body->getResponse());

        $envelope = new ModifyAppointmentEnvelope($body);
        $this->assertSame($body, $envelope->getBody());
        $envelope = new ModifyAppointmentEnvelope();
...
<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope" xmlns:urn="urn:zimbraMail">
    <soap:Body>
        <urn:ModifyAppointmentRequest id="$id" comp="$componentNum" ms="$modifiedSequence" rev="$revision" echo="true" max="$maxSize" want="true" neuter="true" forcesend="true">
...
EOT;
        ...
        $this->assertEquals($envelope, $this->serializer->deserialize($xml, ModifyAppointmentEnvelope::class, 'xml'));
    }
}
